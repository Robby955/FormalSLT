...
learning theory. The v0.1 surface has two useful endpoints and two API
reusability checks:
...
4. a finite discrete `Fin n` dyadic-net family with an embedded Rademacher
   process, showing the same wrapper with explicit nonconstant cover-count
   bookkeeping and nontrivial supplied-supremum routing.
