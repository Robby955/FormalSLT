...
status: "draft"
description: "A theorem-faithful TheoremPath draft explaining the checked FormalSLT v0.1 chains: a finite-class Hoeffding confidence sequence and a unit-interval Dudley finite-net bridge."
tags:
  - lean4
...
learning theory. The v0.1 surface has two useful endpoints:
...
1. Use the `FiniteClassConfidenceSequence` bundle in downstream notes and
   route metadata instead of restating the long theorem signature.
2. Abstract the rounded dyadic-net Dudley wrapper so the unit-interval proof is
