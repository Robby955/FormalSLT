...
4. a finite discrete `Fin n` dyadic-net family, showing the same wrapper with
   explicit nonconstant cover-count bookkeeping.
...
- The finite discrete `Fin n` dyadic-net family is covered by
...
2. Replace the finite-discrete zero-process example with a nonzero finite
   process, so the finite-family API check also exercises a nontrivial
   supremum.
