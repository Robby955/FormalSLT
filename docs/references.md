...
| PAC-Bayes Bernstein and Gaussian/variational interfaces | Tolstikhin and Seldin (2013); Alquier, Ridgway, and Chopin (2016) | Variance-sensitive penalties, the source-normalized finite empirical-variance MGF, and continuous-posterior analytic interfaces | The Tolstikhin--Seldin empirical-variance moment and its fixed-tilt finite-posterior variance-confidence lift are checked for finite IID `[0,1]` losses; other generic interfaces retain their explicit supplied certificates |
