...
|---|---|---|---|---|---|---|---|
| All-sample-size empirical-Bernstein PAC-Bayes | `PACBayes.InfiniteEmpiricalBernsteinStitch.exists_infiniteEmpiricalBernstein_event`; finite-epoch endpoint: `PACBayes.FiniteEmpiricalBernsteinReverseSqrt.exists_finiteEmpiricalBernsteinReverseSqrt_event`; load-bearing source MGF: `PACBayes.FiniteEmpiricalVarianceMGF.finiteEmpiricalVariance_lowerTailMGF_tolstikhinSeldin` | Tolstikhin and Seldin, [PAC-Bayes-Empirical-Bernstein Inequality](https://proceedings.neurips.cc/paper/2013/file/a97da629b098b75c294dffdc3e463904-Paper.pdf), especially Eq. (9); compare the established anytime PAC-Bayes frameworks of Chugg, Wang, and Ramdas, [A Unified Recipe for Deriving (Time-Uniform) PAC-Bayes Bounds](https://www.jmlr.org/papers/volume24/23-0401/23-0401.pdf), and Jang, Jun, Kuzborskij, and Orabona, [Tighter PAC-Bayes Bounds through Coin-Betting](https://proceedings.mlr.press/v195/jang23a/jang23a.pdf) | **SPECIALIZATION** for the finite-IID Bessel-MGF foundation; **DERIVED VARIANT** for the reverse-epoch martingale, dyadic stitching, and final all-sample-size endpoint | IID `[0,1]` losses; posterior average of per-hypothesis Bessel variances; one fixed full-support prior; one KL term; one measurable event valid for every `n >= 2` and every finite posterior, including a path-selected posterior | Finite data and hypothesis types. The displayed constants are `5/2` on `sqrt(Vhat * L / n)` and `5` on `L/n`, with `L = KL + log(r(r+1)^2/delta)` and `r = Nat.log 2 n`. The construction is an offline reverse-exchangeability/reverse-epoch theorem, not a literal reproduction of a cited paper theorem, a forward e-process, an optional-stopping theorem, an all-real optimizer, or a continuous-hypothesis result. No novelty or priority claim is made | `CheckInfiniteEmpiricalBernsteinStitch.fairBool_allTime_selectedPosterior_receipt`: one event of mass at most `1/20` for all `n >= 2` and the fair-Boolean posterior selected from the first observation. The balanced-64 fixed-sample receipt separately checks positive KL, positive Bessel variance, positive sample mass, and a final ceiling below one | Publish a symbol-by-symbol source-to-Lean derivation; compare the stitched boundary against PAC-Bayes-kl, Hoeffding/Catoni, Tolstikhin--Seldin, and existing anytime PAC-Bayes boundaries on identical inputs; map the all-`n` nonvacuity region; obtain PAC-Bayes and Lean probability reviews |
