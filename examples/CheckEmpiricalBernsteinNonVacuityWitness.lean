...
`condExp_of_stronglyMeasurable`, so the only admissible process was the zero
process and the bound was vacuous. The `+1` shift in `IncrementAdapted` admits the
genuine `±1` increment here, while the variance proxy `V` keeps the past-conditioning
`StronglyAdapted ℱ V` it should have.

The instance uses `b = 1`, `delta = 1/2`, `lam0 = 1/2`, `lam1 = 1`
...
namespace FormalSLT.AnytimeValid.EmpiricalBernsteinNonVacuityWitness
...
/-- The constant variance proxy `V ≡ 1`, a valid predictable bound on `μ[X² | ℱ]`. -/
def VBool : ℕ → Bool → ℝ :=
  fun _ _ => 1
...
theorem nonVacuityWitness :
    (μBool).real {ω | ∃ n : ℕ, 0 < n ∧
        (1 / (1 / 2 : ℝ)) ≤
          empiricalBernsteinMixtureProcess XBool VBool 1
            (uniformTiltPrior (1 / 2) 1) n ω}
      ≤ (1 / 2 : ℝ) := by
  refine empiricalBernstein_confidence_sequence_uniformPrior
    (μ := μBool) (ℱ := filtBool) (X := XBool) (V := VBool)
    (b := 1) (delta := 1 / 2) (lam0 := 1 / 2) (lam1 := 1)
  ...
  · -- hX_adapted : IncrementAdapted filtBool XBool
    intro k
    have hk : filtBool (k + 1) = ⊤ := by
      simp [filtBool]
    rw [show (StronglyMeasurable[filtBool (k + 1)] (XBool k)) =
        (StronglyMeasurable[⊤] (XBool k)) from by rw [hk]]
    exact (measurable_from_top).stronglyMeasurable
  · -- hV_adapted : StronglyAdapted filtBool VBool
    intro k
  ...
  · -- hvar : μ[X k ^ 2 | ℱ k] ≤ V k
    ...
    · subst hk
      ...
      exact Filter.Eventually.of_forall (by intro ω; norm_num [VBool])
    · have hXk : XBool k = 0 := by
        funext ω; simp only [XBool, if_neg hk]; rfl
...
end FormalSLT.AnytimeValid.EmpiricalBernsteinNonVacuityWitness
