...
| `PACBayes` | Change of measure, KL calculations, bounded-loss and Bernstein bounds, finite exponential tilting and product identities, finite empirical-variance foundations plus random-matching MGF, fixed-tilt posterior confidence, standalone weighted variance/risk catalogs, a retained-Bennett joint mean/Bessel-variance MGF, a one-event finite joint-pair posterior catalog over explicit product weights, its exact attained three-branch residual envelope on `[0,1/4]`, a closed-form logarithmic-grid empirical-Bernstein endpoint, and a support-aware fixed-sample countable master-mixture foundation with a finite-posterior exact-`xi` selector over predeclared `Nat`-indexed tilt pairs; process-level time-uniform results, including a finite normalized hypothesis--tilt master e-process with selected-atom penalties and fixed prior/posterior pairs on arbitrary measurable hypothesis spaces; and finite-class or finite-dimensional spherical-Gaussian i.i.d. results at discrete sample times, including finite data-dependent posterior/tilt selection where stated. |
