...
| `PACBayes` | Change of measure, KL calculations, bounded-loss and Bernstein bounds, finite empirical-variance foundations plus random-matching MGF, fixed-tilt posterior-confidence, a standalone finite weighted empirical-variance tilt catalog, fixed-parameter empirical-Bernstein risk, and a final separately weighted finite `eta`/`lambda` catalog over explicit product weights, process-level time-uniform results for fixed prior/posterior pairs on arbitrary measurable hypothesis spaces, and finite-class or finite-dimensional spherical-Gaussian i.i.d. results at discrete sample times, including finite data-dependent posterior/tilt selection where stated. |
