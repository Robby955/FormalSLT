...
# Validate the prospective real-data Brier monitor preregistration and fail if
# any declared fresh stream, receipt, manifest, or generated Lean output
# exists. This target downloads nothing.
check-gjp-brier-protocol:
	python3 scripts/check_gjp_brier_protocol.py

verify-gjp-brier-protocol: check-gjp-brier-protocol
	python3 -m pytest -q tests/test_gjp_brier_protocol.py

# Digest-verify the pinned Good Judgment Project inputs into a directory
# outside the repository. Requires network access and GJP_INPUT_DIR.
fetch-gjp-brier-inputs:
	python3 scripts/fetch_gjp_brier_inputs.py --out "$(GJP_INPUT_DIR)"

# Regenerate the compact Lean arithmetic inputs from an external replay.
# Requires GJP_REPLAY_DIR containing the canonical stream and receipt JSON.
generate-gjp-brier-lean-data:
	python3 scripts/generate_gjp_brier_lean_data.py \
		--stream "$(GJP_REPLAY_DIR)/gjp-brier-monitor-v1-stream.json" \
		--receipt "$(GJP_REPLAY_DIR)/gjp-brier-monitor-v1-receipt.json"
	python3 scripts/generate_gjp_brier_lean_path.py \
		--stream "$(GJP_REPLAY_DIR)/gjp-brier-monitor-v1-stream.json" \
		--receipt "$(GJP_REPLAY_DIR)/gjp-brier-monitor-v1-receipt.json"

# Check the external replay binding, generated bytes, and kernel-checked
# arithmetic. This does not discharge named-path good-event membership.
verify-gjp-brier-lean-arithmetic:
	python3 scripts/generate_gjp_brier_lean_data.py \
		--stream "$(GJP_REPLAY_DIR)/gjp-brier-monitor-v1-stream.json" \
		--receipt "$(GJP_REPLAY_DIR)/gjp-brier-monitor-v1-receipt.json" \
		--check
	lake build FormalSLT.Applications.GJPBrierMonitorReplayReceipt
	lake env lean examples/CheckGJPBrierMonitorReplayReceipt.lean

# Check every pathwise generated module sequentially to cap peak memory, then
# verify the full score/path theorem instance and explicit event-mass result.
verify-gjp-brier-lean-path:
	...
	python3 scripts/generate_gjp_brier_lean_path.py \
		...
		--receipt "$(GJP_REPLAY_DIR)/gjp-brier-monitor-v1-receipt.json" \
		--check
	lake build FormalSLT.Applications.GJPBrierMonitorReplayPathDataBase
	lake build FormalSLT.Applications.GJPBrierMonitorReplayPathDataConstantTrainBaseRate
	lake build FormalSLT.Applications.GJPBrierMonitorReplayPathDataFirstWeekMean
	lake build FormalSLT.Applications.GJPBrierMonitorReplayPathDataFinalConsensusMedian
	lake build FormalSLT.Applications.GJPBrierMonitorReplayPathDataExtremizedFinalConsensus
