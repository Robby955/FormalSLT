...
class ServiceState:
    ...
    def create(self, protocol: dict[str, JsonValue]) -> tuple[str, MonitorSession]:
        ...
        session = MonitorSession(monitor=monitor)
        with self._lock:
...
def _issue_session_certificate(
    ...
    if cached is not None:
        _path, certificate, selection = cached
        return f"{monitor_id}:{observations}", certificate, selection
    ...
    return certificate_id, certificate, frozen.record
...
def create_app(output_root: Path | None = None) -> FastAPI:
    ...
    def certify_monitor(monitor_id: str) -> CertificateResponse:
        ...
        with session.lock:
            try:
                certificate_id, certificate, selection = _issue_session_certificate(
                    state,
                    monitor_id,
                    session,
                )
    ...
    def get_certificate(monitor_id: str, observations: int) -> CertificateResponse:
        ...
        with session.lock:
            ...
            _certificate_path, certificate, selection = stored
            return CertificateResponse(
                certificate=certificate,
                certificate_id=f"{monitor_id}:{observations}",
                monitor_id=monitor_id,
