...
FRONTIER_LANES: list[dict[str, str]] = [
    {
        ...
        "scope": (
            ...
            "measurable cylinders, and terminal-state occupancy. A separate "
            "supplied jointly "
            ...
            "homogeneous finite-state Markov weighted-tilt certificate also "
        ...
        "next_step": (
            ...
            "sample splitting. Extend random starts beyond the homogeneous "
            "finite-state Markov endpoint to the general prefix-dependent, "
            "measurable-state, and controlled layers. Add unvisited-row and "
            "quantitative mixing interfaces, and an anytime-valid cumulative-"
            "weight or target-policy value theorem. Integrate the exact finite "
            "and countable sleeping-expert betting masters with the measurable "
            "growing-prefix certificate, then extend beyond the fixed dyadic "
            "sleeping catalog toward parameter-free coin betting. Extend the "
            "already checked "
        ...
        "boundary": (
            ...
            "scored outcomes or supply arbitrary joint predictor--tilt "
            "posteriors, unrestricted all-real tilt control, multistep "
            "prediction, or optional stopping. The separate "
            ...
            "posterior and tilt and still uses two-atom dynamics. The "
            ...
            "master-process claim. A separate real-tsum master and growing-"
            "prefix oracle support arbitrary measurable hypothesis and state "
            "spaces, eligible continuous posteriors, ordinary monitored-risk "
            "semantics, and an observable LIL-order envelope. Its vanishing "
            "width requires the displayed posterior-KL rate, and its exact "
            "real-valued argmin is noncomputable. These "
            ...
            "posteriors on measurable spaces, all-real tilt control, multistep "
            "prediction, optional "
            "stopping, matched boundary comparison, or stationary long-run "
            "risk. Random initial "
            "laws are checked only for the homogeneous finite-state Markov "
            "weighted-tilt certificate."
        ),
    },
    {
    ...
    },
    {
        ...
        "scope": (
            ...
            "computable finite wealth-weighted predictable-strategy master and "
            "an exact countable dyadic sleeping-expert master whose active "
            "prefix and closed-form tail equal the real infinite mixture; a "
            "forward continuous-prior master over arbitrary measurable "
            "hypotheses with both finite and countable tilt catalogs, an "
            "observable growing-prefix oracle, and an arbitrary-measurable-"
            "state trajectory adapter under a supplied joint score contract; "
            "and "
        ...
        "next_step": (
            "Extend the finite joint model--strategy posterior theorem to "
            "measurable hypothesis spaces. Integrate the exact finite and "
            "countable sleeping-expert masters with PAC-Bayes risk endpoints, "
            "then extend their automatic competition beyond the fixed dyadic "
            "sleeping catalog toward parameter-free coin betting and support "
        ...
        "boundary": (
            ...
            "and tilt. The reverse stitched theorem is posterior-uniform on an "
            "arbitrary measurable hypothesis space, but it is an offline "
            "reverse-exchangeability result. The forward hybrid-Bessel master "
            ...
            "vanishing width. A separate real-tsum construction lifts the "
            "countable master and growing-prefix oracle to arbitrary measurable "
            "hypothesis and state spaces with eligible continuous posteriors. "
            "Its vanishing result requires the displayed posterior-KL rate. "
            "Exact finite and countable sleeping-expert wealth-weighted masters "
            "are computable over rational inputs and compete with every active "
            "declared expert. They are not yet integrated into the PAC-Bayes "
            "risk endpoint and do not provide a countably supported strategy "
            "posterior or automatic competition with a continuum of legal "
            "strategies. The hybrid expression is "
            "only a pointwise lower envelope. The arbitrary-state adapter "
            "starts deterministically "
            "and does not construct a measurable selector or selected process. "
            "The Gaussian/fair-Boolean receipt fixes posterior and tilt, has "
            "KL = 1/32 and boundary at most 489/1024, and uses two-atom "
    ...
    },
    {
        ...
        "scope": (
            ...
            "and a continuous-prior countable master over arbitrary measurable "
            "hypotheses with a growing-prefix oracle and an arbitrary-"
            "measurable-state full-prefix trajectory adapter under a supplied "
            "joint score contract, exact finite and countable sleeping-expert "
            "wealth-weighted betting masters, plus a finite-state full-prefix trajectory "
            "adapter with countable confidence allocation and the same "
            "vanishing selected-boundary conclusion"
        ),
        "difficulty": "hard",
        "source": (
        ...
        "next_step": (
            "Extend the finite joint model--strategy posterior theorem to "
            "measurable hypothesis spaces. Integrate the exact computable "
            "finite and countable sleeping-expert masters with the PAC-Bayes "
            "risk endpoint, then pursue parameter-free competition, countably "
            "supported strategy posteriors, broader "
        ...
        "boundary": (
            ...
            "risk semantics. A separate real-tsum master and oracle lift these "
            "features to arbitrary measurable hypothesis and state spaces with "
            "eligible continuous posteriors; vanishing width then requires the "
            "displayed pathwise posterior-KL rate. A separate finite product-"
            "catalog theorem supports "
            "post-data model and strategy posterior PMFs with separate KL costs. "
            "Separate exact finite and countable sleeping-expert masters provide "
            "automatic wealth competition over their declared catalogs, but are "
            "not yet composed with this risk theorem, do not compete over all "
            "legal strategies, and do not support a countably infinite strategy "
            "posterior. A separate finite-state "
            "trajectory theorem obtains the same selected conclusion by "
            "countable confidence allocation, not a countable master-process "
            "claim. Its continuous-prior master permits arbitrary measurable "
            "hypotheses and a positive normalized countable tilt catalog, and "
            "has an arbitrary-measurable-state trajectory adapter under "
