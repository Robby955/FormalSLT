...
namespace FormalSLT.Rademacher.Contraction
...
theorem one_step_contraction {φ : ℝ → ℝ}
  ...
  -- add_sum_erase puts the j-th term first; we swap with add_comm.
...
/-- Partially apply `φ` to the first `m` coordinates: coordinates `k` with
...
only at coordinate `⟨m, hm⟩`, where it replaces `g i ⟨m, hm⟩` with `φ(g i ⟨m, hm⟩)`. -/
...
end FormalSLT.Rademacher.Contraction
