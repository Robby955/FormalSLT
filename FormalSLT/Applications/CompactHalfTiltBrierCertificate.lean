...
an exact empirical Brier loss, an exact forward-predictor quadratic variation,
and rational logarithm witnesses.  Lean then checks the KL bound, confidence
term, cumulant penalty, and final endpoint without embedding every input row.
...
/-- Exact summary endpoint appearing in the half-tilt trajectory theorem. -/
def summaryEndpoint
...
with exact nonnegative quadratic variation and rational endpoint arithmetic. -/
